...
class Client implements Runnable {
	...
	public void run() {
		for (int i = 0; i < ConcDS.N; i++) {
			...
			if (r <= 2) {
				ConcDS.cache
						.put(DNAStrings.data[index], DNAStrings.data[index]);
				// 1% remove
			} else if (r <= 3) {
...
class ConcurrentHashMapCache implements Cache {
	...
	/**** START ****/
	Map<String, String> concurrentCache = new ConcurrentHashMap<String, String>();
	...
	@Override
	public boolean containsKey(String k) {
		return concurrentCache.containsKey(k);
	}
