    ...
    while (true) {
      ...
      if (cmd[0].equals("quit") || cmd[0].equals("q")) {
        ...
        if (mapUsers.containsKey(cmd[1])) {
          System.out.println("User %s already present, ignoring add user\n" + cmd[1]);
          continue; 
        }
        
        System.out.println("Adding User " + cmd[1] + " to the Map of Users");
        ...
        if (user == null) {
          System.out.printf("User %s does NOT exist, ignoring subscribe\n", cmd[1]);
          continue; 
        }
